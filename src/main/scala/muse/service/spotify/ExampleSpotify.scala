...
import muse.domain.spotify.{BulkPlaylist, PlaylistTrack, Track}
...
object ExampleSpotify extends ZIOAppDefault {
  val program = for {
    ...
    refCompat          = Givens.zioRef(ref)
    spotify            = SpotifyAPI[Task](backend, refCompat, clientCredentials.accessToken)
...
case class WeeklyAnalysis(spotify: SpotifyAPI[Task]) {
  ...
  def getWeeklyPlaylists: Task[Vector[(BulkPlaylist, LocalDate)]] = for {
    playlists   <-
