...
object RedisService {
  ...
  def get[K: Schema, V: Schema](key: K)                                         =
    ZIO.serviceWithZIO[RedisService](_.get[K, V](key))
  def set[K: Schema, V: Schema](key: K, value: V, expiration: Option[Duration]) =
    ZIO.serviceWithZIO[RedisService](_.set(key, value, expiration))
  def delete[K: Schema](key: K)                                                 =
    ZIO.serviceWithZIO[RedisService](_.delete(key))
  def rateLimited(identifier: String)                                           =
    ZIO.serviceWithZIO[RedisService](_.rateLimited(identifier))
  ...
  // Make connection reloadable in case timeouts start happening.
  def redisLayer = ((SingleNodeExecutor.layer ++ codecLayer) >>> Redis.layer.tap { redis =>
    for {
